...
async def _load_entities_from_uploads(
    ...
    _nola_bd_leu = {"assessments": 0, "interest": 0, "late_fees": 0, "atty_fees": 0, "other": 0}
        ...
        for _il in nola_text.split("\n"):
            ...
            if "interest" in _il_lc and not any(kw in _il_lc for kw in ("total", "balance", "please", "remit")):
                ...
                if _im_leu:
                    ...
                    break
        _nola_bd_leu = {
            "assessments": float(_nola_cats_leu.get("maintenance", 0)) + float(_nola_cats_leu.get("special_assessments", 0)),
...
async def generate_ledger(request: LedgerRequest):
    ...
    # entirely to "assessments", but the NOLA balance actually contains mixed
    ...
    if nola_balance_anchor and nola_charge_breakdown:
        ...
        _nola_asmts   = float(_nb.get("assessments", 0))
        _nola_int     = float(_nb.get("interest", 0))
        _nola_late    = float(_nb.get("late_fees", 0))
        _nola_atty    = float(_nb.get("atty_fees", 0))
        _nola_other   = float(_nb.get("other", 0))
        # The NOLA row's full balance was lumped into totals["assessments"].
        # Remove it and re-add by category.
        totals["assessments"] = round(totals["assessments"] - nola_balance_anchor + _nola_asmts, 2)
        totals["interest"]    = round(totals["interest"] + _nola_int, 2)
        totals["late_fees"]   = round(totals["late_fees"] + _nola_late, 2)
        totals["atty_fees"]   = round(totals["atty_fees"] + _nola_atty, 2)
    ...
    def _build_excel():
        ...
        with pd.ExcelWriter(str(out_path), engine="openpyxl") as writer:
            ...
            if nola_balance_anchor is not None:
                # totals already contain properly categorized NOLA + post-NOLA amounts
                # (redistributed via nola_charge_breakdown above)
                _nb = nola_charge_breakdown or {}
                _nola_asmts_only = float(_nb.get("assessments", 0))
                _post_nola_asmts = round(totals["assessments"] - _nola_asmts_only, 2)
                _nola_int   = float(_nb.get("interest", 0))
                _nola_late  = float(_nb.get("late_fees", 0))
                _post_nola_int  = round(totals["interest"] - _nola_int, 2)
                _post_nola_late = round(totals["late_fees"] - _nola_late, 2)
                _nola_atty_only = float(_nb.get("atty_fees", 0))
                _nola_other_only = float(_nb.get("other", 0))
                _post_nola_atty  = round(totals["atty_fees"] - _nola_atty_only, 2)
                _post_nola_other = round(totals["other"] - _nola_other_only, 2)
                soa_rows = [
                    ("NOLA Opening Balance",                               nola_balance_anchor),
                    (f"Post-NOLA Assessments through {_through_lbl_soa}", max(0.0, _post_nola_asmts)),
                    ("Post-NOLA Interest accrued (18% per annum)",         max(0.0, _post_nola_int)),
                    ("Post-NOLA Administrative Late Fees",                 max(0.0, _post_nola_late)),
                    ("Attorney's Fees",                                    max(0.0, _post_nola_atty)),
                    ("Other Charges",                                      max(0.0, _post_nola_other)),
